<template>
    <div>
        ...
        <aside :class="[
            'fixed md:relative top-0 left-0 h-full bg-gray-50 dark:bg-[#162d38] border-r border-gray-200 dark:border-neutral-700 flex flex-col transform transition-transform duration-300 z-50',
            isOpen ? 'translate-x-0' : '-translate-x-full',
            'md:translate-x-0 w-64'
        ]">
            <!-- Header -->
            <header class="p-4 flex justify-between items-center bg-[#162d38]">
            ...
            <!-- Perfil -->
            <div class="flex items-center gap-2 p-4 border-b border-gray-100 dark:border-neutral-700">
                ...
                <div>
                    <p class="text-xs">
                        <strong class="block font-medium text-gray-900 dark:text-white">Sebastián Suazo</strong>
                        <span class="text-gray-500 dark:text-neutral-300 text-xs">suazosebastian68@gmail.com</span>
                    </p>
                </div>
            </div>
            ...
            <nav class="flex-1 overflow-y-auto px-2 pb-4 space-y-1">
                <ul class="space-y-1">
                    <li v-for="item in menuItems" :key="item.text">
                        <a :href="item.href"
                            class="flex items-center gap-x-3.5 py-2 px-2.5 rounded-lg text-sm text-gray-800 hover:bg-gray-100 dark:text-white dark:hover:bg-[#ff6600]">
                            {{ item.text }}
                        </a>
                    </li>
                </ul>
            ...
            <div class="px-2 py-2 border-b border-gray-100 dark:border-neutral-700">
                <a @click.prevent="logout"
                    class="w-full group relative inline-flex items-center overflow-hidden gap-x-3.5 py-2 px-2.5 rounded-lg text-sm text-gray-800 hover:bg-gray-100 dark:text-white dark:hover:bg-[#ff6600] cursor-pointer ">
                    <!-- Flecha animada -->
